...
class DWN(nn.Module):
    ...
    def forward_fixed_gamma(self, u, state=None, mode="scan", gammaT=None):
        ...
        decoder = gamma_d * self.decoder / (torch.norm(self.decoder, 2) * torch.norm(self.encoder, 2))
        x = u @ encoder.T
        for layer, block in enumerate(self.blocks):
            state_block = state[layer] if state is not None else None
            x, st = block(x, gamma=gamma_mid[layer] - 1, state=state_block, mode=mode)
        x = x @ decoder.T
    ...
    def forward(self, u, state=None, mode="scan", gamma=None):
...
class PureLRUR(nn.Module):
    ...
    def forward(self, x, gamma=None, state=None, mode: str = "scan"):
        y, st = self.lru(x, gamma=gamma, state=state, mode=mode)

        return y, st


class PureLRU(nn.Module):
    """ Just LRU """

    def __init__(self, config: DWNConfig):
        super().__init__()
        self.lru = LRU(config.d_model, config.d_model, config.d_model)
